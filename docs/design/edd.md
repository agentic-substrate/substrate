...
- **Phase 1:** `SessionStart` → `context.get` via server (or cache) → prints pack to stdout for injection. `PostToolUse` → appends a compact observation to `outbox` (tool, files, exit status, ≤ 500 chars). This is the minimum capture needed to prove cross-machine sync (PRD SYNC-2, CAP-1a).
...
| R20 | High | Hook capture split inconsistently between Phase 1 and 2 | Phase 1 = `SessionStart` + `PostToolUse`→outbox (CAP-1a); Phase 2 = `Stop`/`PreCompact` summarization + checkpoints (CAP-1b). PRD amended |
