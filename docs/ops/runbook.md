...
| 10 | `00010_scope_readable.sql` | Adds `scope_readable(scope_id, actor)`, the read-side predicate `resolveRepoPaths` evaluates before it hands a repo key's chain back to `/v1/render`, `/v1/skills/manifest` or `/v1/memory/cache`. `scope` carries no RLS, so before this a foreign-but-existing repo key answered 200 (with content filtered by row RLS) while a nonexistent one answered 403, and the status code alone enumerated which repo keys this control plane binds. It delegates to `scope_writable` — `member_role` has no read-only tier, so no grant confers read without write today — which also inherits the `substrate.is_admin` short-circuit, so a `human_admin` still resolves every key. A read-only grant role, if one is ever added, widens this function alone. |
