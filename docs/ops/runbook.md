...
| 10 | `00010_scope_readable.sql` | Adds `scope_readable(scope_id, actor)`, the read-side predicate `resolveReadableRepoPaths` evaluates before it hands a repo key's chain to `/v1/render`, `/v1/skills/manifest` or `/v1/memory/cache`. `scope` carries no RLS, so before this a foreign-but-existing repo key answered 200 while a nonexistent one answered 403, and the status code alone enumerated which repo keys this control plane binds. A false result is **not** a refusal on the read paths: the key resolves to nothing, the handler falls back to the global chain, and both the foreign and the nonexistent key return 200 with the byte-identical body the same request produces with no `?repos=` — equal answers, not equal denials, which is what kills the enumeration oracle. What that fallback preserves is only the content the global chain itself reaches: on `/v1/memory/cache`, whose no-`?repos=` path applies no scope filter and relies on RLS alone, that includes every `visibility='global'` row regardless of where it was authored. On `/v1/render` and `/v1/skills/manifest`, whose no-`?repos=` path still resolves a scoped (global-only) chain, a `visibility='global'` row authored *below* global — reachable only through the named repo's chain — is withheld from a caller who cannot read that chain, exactly as the old 403 withheld it; the fallback changes the status code there, not the content. Delivering that below-global content to a caller who cannot read the chain would require querying the chain to find it, which reopens the oracle this migration closes — so the remaining withholding on those two endpoints is inherent to the design, not a shortfall of it. The accepted cost is that a member who typos their own repo key silently gets global content instead of an error. The write paths are unchanged: `h.repoScope` keeps `scope_writable` and its masked 403. It delegates to `scope_writable` — `member_role` has no read-only tier, so no grant confers read without write today — which also inherits the `substrate.is_admin` short-circuit, so a `human_admin` still resolves every key. A read-only grant role, if one is ever added, widens this function alone. |
| 11 | `00011_seed_global_scope.sql` | Inserts the single `global` scope row if none exists. Every read path resolves the global chain, and before this only test fixtures created that row, so a fresh install answered `500 lookup global: no rows in result set` on `/v1/render` — the request `substrate auth login` probes with. Down is a deliberate no-op: deleting the root would orphan every chain beneath it. |
...
| 4 | Substitute `<cnpg-base-image-digest>` in [`deploy/cnpg/Dockerfile`](../../deploy/cnpg/Dockerfile) (`docker buildx imagetools inspect ghcr.io/cloudnative-pg/postgresql:16-system-trixie` prints the digest — **system**, the only flavor that ships the barman-cloud binaries the in-tree backup config execs; on `standard` WAL archiving fails while the Cluster still reports healthy), then build **and push**: `docker build -t <registry>/substrate-pg:16-pgvector deploy/cnpg/ && docker push <registry>/substrate-pg:16-pgvector`. Read the pushed digest back (`docker buildx imagetools inspect <registry>/substrate-pg:16-pgvector`) and put `<registry>/substrate-pg:16-pgvector@sha256:…` — **tag and digest** — into `imageName` in `cluster.yaml`, `scratch-cluster.yaml`, and the `<substrate-pg-image>` comparison in [`admission-policy.yaml`](../../deploy/restore/admission-policy.yaml). A locally-loaded image is not enough: step 2's gate requires a digest, and containerd on the node cannot see your local Docker daemon. A bare `@sha256:` is refused by CNPG's webhook ("can't detect upgrades"), and the tag must start with the PG major — CNPG parses it from the leading number, so a git-SHA tag reads as the wrong major. | Image store and registry only. |
