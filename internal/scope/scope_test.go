...
func TestParseRoundTrip(t *testing.T) {
	...
	const in = "global:/org:acme/project:plotlens/repo:plotlens%2Fapi"
	p := mustParse(t, in)
...
func TestContainsIsTheApplicabilityRule(t *testing.T) {
	item := mustParse(t, "global:/project:plotlens/repo:plotlens%2Fapi")
	branch := mustParse(t, "global:/project:plotlens/repo:plotlens%2Fapi/branch:feature-x")
	other := mustParse(t, "global:/project:plotlens/repo:plotlens%2Fweb")
...
func TestAncestorsAreMostSpecificFirst(t *testing.T) {
	p := mustParse(t, "global:/org:acme/project:plotlens/repo:plotlens%2Fapi")
	got := p.Ancestors()
	if len(got) != 4 {
		t.Fatalf("len(Ancestors()) = %d, want 4", len(got))
	}
	want := []Kind{Repo, Project, Org, Global}
	for i, k := range want {
